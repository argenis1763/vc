        ...
        if (isset($_POST[CrugeUtil::config()->postNameMappings['CrugeStoredUser']])) {
            ...
            if ($model->validate()) {
                ...
                if (Yii::app()->user->um->save($model, 'insert')) {
                    ...
                    $usuario = Yii::app()->user->um->loadUser($model->username,true);
                    $sql = "call procedureInsert_StudentsParents("
                            . "'".Yii::app()->user->um->getFieldValue($model->iduser,'firstname')."',"
                            . "'".Yii::app()->user->um->getFieldValue($model->iduser,'lastname')."',"
                            . "'".$usuario->email."',"
                            . "'".Yii::app()->user->um->getFieldValue($model->iduser,'phone')."',"
                            . $usuario->iduser.");";   
